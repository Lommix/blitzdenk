...
pub const default_tool_set = .{
    ...
    .{ tools.agent.ReadBackgroundAgent, ToolFlags.all },
    .{ tools.agent.CancelBackgroundAgent, ToolFlags.all },
...
pub const ToolFlags = struct {
    ...
    pub fn agents(self: Self, types: []const AgentType) Self {
        var s = self;
...
// Central reg for all agent related tool sets and prompts

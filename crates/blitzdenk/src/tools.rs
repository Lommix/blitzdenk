...
impl AiTool for PatchFile {
    ...
    fn description(&self) -> &'static str {
        ...
        **Importance of Correct Patch File Format:**
        ...
        A **`.diff` file (Unified Format)** describes changes between two file/directory states ("original" and "modified") for the `patch` command to apply.

        **Key Components:**

        1.  **File Headers:**
            *   `--- a/path/to/original_file[timestamp]` (Original file)
            *   `+++ b/path/to/modified_file[timestamp]` (Modified file)
            *   `a/` and `b/` are conventional prefixes.

        2.  **Change Hunks (for file content):**
            *   Starts with `@@ -old_start,old_lines +new_start,new_lines @@ [optional_section_heading]`
                *   Specifies line numbers and count for original (`-`) and modified (`+`) versions.
            *   Contains context lines and actual changes.
        ...
        4.  **File Creation / Deletion:**
            *   **Creation:** Original file is `/dev/null` (or similar empty placeholder).
                *   Example: `--- /dev/null` and `+++ b/path/to/new_file.txt`.
                *   All content lines in hunks are prefixed with `+`.
            *   **Deletion:** Modified file is `/dev/null`.
                *   Example: `--- a/path/to/old_file.txt` and `+++ /dev/null`.
                *   All content lines in hunks are prefixed with `-`.

        5.  **Directory Changes (Implicit):**
            *   **Creation:** New directories are implicitly defined by new file paths in `+++` headers (e.g., `+++ b/new_dir/file.txt`). `patch` often creates them.
            *   **Deletion:** The format does not explicitly represent directory deletion; this must be handled externally.

        6.  **Special Marker (Optional):**
            *   `\ No newline at end of file`: Indicates the preceding line in the hunk (original or modified part) lacked a final newline character.

        7.  **Git Diff Extensions (Common):**
            *   `git diff` may add extra headers like:
                *   `new file mode <mode>`
            ...
            *   These provide richer metadata that `patch` might interpret or that other tools use.
        "#
    }

...
impl AiTool for PatchFile {
    ...
    fn description(&self) -> &'static str {
        ...
        **Importance of Correct Patch File Format:**
        ...
        A `.diff` file in Unified Format describes changes between two file/directory states (an "original" and a "modified" version) so the `patch` command can apply these changes.
        ...
        **Key Components:**
        ...
        1.  **File Headers:**
            ...
            *   These identify the files. `a/` and `b/` are conventional prefixes. Timestamps may follow.

        2.  **Change Hunks (for file content):**
            *   Starts with `@@ -old_start,old_lines +new_start,new_lines @@ [optional_section_heading]`
                *   `-old_start,old_lines`: Location in the *original* file.
                *   `+new_start,new_lines`: Location in the *modified* file.
            *   Contains actual changes and surrounding context lines.

        3.  **Line Prefixes within Hunks:**
            *   ` ` (a space): **Context line** (unchanged).
            *   `-`: **Deletion** (line removed from original).
            *   `+`: **Addition** (line added to modified).

        4.  **File Creation:**
            *   Represented by using `/dev/null` (or a similar empty placeholder) as the "original" file.
            *   Header:
                ```diff
                --- /dev/null
                +++ b/path/to/new_file.txt
                ```
            *   All lines in the hunk(s) will be prefixed with `+`.
            *   `patch` will create `path/to/new_file.txt`. If parent directories (e.g., `path/to/`) don't exist, `patch` (often with the `-p` option to strip leading path components) will attempt to create them if the diff was generated with directory context (e.g., `diff -Naur old_dir new_dir`).

        5.  **File Deletion:**
            *   Represented by using `/dev/null` as the "modified" file.
            *   Header:
                ```diff
                ...
                ```
            *   All lines in the hunk(s) will be prefixed with `-`.
            *   `patch` will delete `path/to/old_file.txt` (if it exists).

        6.  **Directory Operations:**
            *   **Creation:** `patch` itself doesn't have a direct "create directory" command within the diff. However, when creating *new files* (as above), `patch` will often create necessary parent directories if they don't exist, especially when the patch was generated recursively (e.g., `diff -Naur old_dir new_dir`) and the `-p<num>` option is used appropriately with `patch`.
            *   **Deletion:** `patch` does **not** delete directories, even if all files within them are deleted by the patch. Directory removal must be handled separately.

        7.  **Special Marker (Optional):**
            *   `\ No newline at end of file`: Indicates if the last line in that part of the hunk lacked a newline.

        **Git Diff Extensions:** Diffs generated by `git diff` may include extra headers like `new file mode ...` or `deleted file mode ...` which `patch` can often interpret to understand file creation/deletion more explicitly.

        **How `patch` uses it:**
        `patch` reads headers to identify files/operations. For content changes, it uses hunk headers and context lines to locate where to apply deletions (`-`) and additions (`+`). For file creation/deletion, it acts on the `/dev/null` convention. It may create parent directories for new files but won't delete directories.
        "#
    }
